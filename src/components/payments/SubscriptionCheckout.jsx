...
const SubscriptionCheckout = ({ planType, amount, onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || "Subscription failed to process");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full bg-[var(--bg-glass)] p-6 rounded-xl border border-[var(--border)] shadow-lg mt-6">
      <h3 className="text-xl font-bold mb-4 text-[var(--text-main)]">Complete Subscription Payment</h3>
      <p className="mb-6 text-[var(--text-soft)]">
        Plan: {planType === "MONTHLY" ? "Monthly" : "Yearly"} (${amount})
      </p>
